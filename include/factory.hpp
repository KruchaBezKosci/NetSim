...
public:
    ...
    // --- wyszukiwanie (testy używają dereferencji iteratora) ---
    ...
    // --- iteratory do przeglądania (read-only) ---
    ...
    // końce kolekcji (przydatne później w IO — część B)
...
// usuwa odbiorcę i sprząta połączenia u wszystkich nadawców
template <typename ReceiverNode>
void Factory::remove_receiver(NodeCollection<ReceiverNode>& collection, ElementID id) {
    ...
    // usuwamy receiver z preferencji ramp i workerów
    for (auto& r : ramps_) {
